...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    const lastModifiedDate = offsetObject.lastModifiedDate || lastPulledDate;
    const lastModifiedDateFilter = generateLastModifiedDateFilter(lastModifiedDate, now, 'engagement.createdAt');
    const searchObject = {
      filterGroups: [
        lastModifiedDateFilter,
        { filters: [{ propertyName: 'engagement.attendanceStatus', operator: 'EQ', value: 'ATTENDED' }] }
      ],
      sorts: [{ propertyName: 'engagement.createdAt', direction: 'ASCENDING' }],
      properties: [
        'engagement.title',
        'engagement.createdAt',
        'engagement.lastUpdated',
        'engagement.attendanceStatus',
        'engagement.contactEmail'
      ],
      engagements: [{ engagementType: 'MEETING' }],
      limit,
      after: offsetObject.after
    };
    ...
    data.forEach((meeting, index) => {
      ...
      const contactEmail = meeting.properties['engagement.contactEmail'];
      const attendanceStatus = meeting.properties['engagement.attendanceStatus']
      const actionName = isCreated ? 'Meeting Created' : 'Meeting Updated';

      const actionTemplate = {
        includeInAnalytics: 0,
        meetingTitle: meeting.properties.engagement.title,
        actionDate: new Date(meeting.lastUpdated),
    ...
    if (!offsetObject?.after) {
      ...
      offsetObject.after = 0;
      offsetObject.lastModifiedDate = new Date(data[data.length - 1].lastUpdated).valueOf();
    }
  }
